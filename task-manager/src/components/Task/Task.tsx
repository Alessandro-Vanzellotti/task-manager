...
export const Task: React.FC<Props> = ({ task, setTaskList }) => {
  ...
  const getPriorityClass = () => {
    switch (task.priorityLevel) {
      case priorityLevelsEnum.none:
        return "priority-none";
      case priorityLevelsEnum.low:
        return "priority-low";
      case priorityLevelsEnum.medium:
        return "priority-medium";
      case priorityLevelsEnum.high:
  ...
  return (
    <div className={"wrapper"}>
      ...
      <div className={"task"}>
        <h2 className={"task__title"} onClick={handleDetails}>
          {" "}
          {task.title}{" "}
        </h2>

        <p className={`task__progress ${getProgressClass()}`}>
          {" "}
          {task.progress}{" "}
        </p>

        <p className={`task__priority-level ${getPriorityClass()}`}>
          {" "}
          {task.priorityLevel}{" "}
        </p>
